...
// @author       YourName (Customized for iOS/Android/Windows)
// @match        *://*/*
// @grant        GM_setValue
// @grant        GM_getValue
// @grant        GM_download
...
const createContextMenu = () => {
    ...
    quickEditBtn.textContent = editMode ? '✖ Quick Edit (ON)' : '✔ Quick Edit (OFF)';
    ...
    htmlEditBtn.textContent = '🛠 HTML Editor';
    ...
    const autoApplyEnabled = storage.get('autoApplyEnabled');
    ...
    autoApplyToggle.addEventListener('click', () => {
        const newState = !storage.get('autoApplyEnabled');
    ...
    downloadBtn.textContent = '⏬ Download Site';
    ...
    removeUIBtn.textContent = '✖ Remove UI';
...
const createHTMLEditor = () => {
    ...
    let wordWrapEnabled = storage.get('wordWrapEnabled', true);
    ...
    wordWrapToggle.addEventListener('click', () => {
        ...
        storage.set('wordWrapEnabled', wordWrapEnabled);
        wordWrapToggle.textContent = wordWrapEnabled ? '☑ Word Wrap' : '☐ Word Wrap';
    ...
    Object.assign(editor.style, {
        ...
        whiteSpace: storage.get('wordWrapEnabled', true) ? 'pre-wrap' : 'pre',
        overflow: 'auto',
        backgroundColor: '#111',
        color: '#fff',
        padding: '10px',
    ...
    applyBtn.addEventListener('click', () => {
        try {
            ...
            // Only apply changes to modified elements to avoid breaking the page
...
if (storage.get('autoApplyEnabled')) {
    ...
    if (storage.get('editModeEnabled')) {
        enableEditMode();
    }
}
})();
